/*
    ...
    a giant sawblade to that height and cuts off all tree parts higher than H (of course, trees not 
    higher than H meters remain intact). Mirko then takes the parts that were cut off. For example, 
    if the tree row contains trees with heights of 20, 15, 10, and 17 metres, and Mirko raises his 
    sawblade to 15 metres, the remaining tree heights after cutting will be 15, 15, 10, and 15 metres, 
...
int isWoodSufficient(long long mid, vector<long long> &treesH, int n, long long requiredWood){
    long long wood = 0;
    for(int i = 0; i < n; ++i){
        if(treesH[i] >= mid){
            wood += (treesH[i] - mid);
...
int main(){
    ...
    long long start = 0;
    long long endd = *max_element(begin(treesH), end(treesH));

    while(endd - start > 1){
        long long mid = start + (endd - start) / 2;
        if(isWoodSufficient(mid, treesH, n, requiredWood)){
            start = mid;
        }
        else{
            endd = mid - 1;
    ...
    if(isWoodSufficient(endd, treesH, n, requiredWood)){
        cout << "Required height setting : " << endd << endl;
    }
    else{
       cout << "Required height setting : " << start << endl;
    }
}